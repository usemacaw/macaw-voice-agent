...
function MetricBar({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className="w-24 text-muted shrink-0 text-right truncate">{label}</span>
  ...
  return (
    <div className="bg-surface border border-border rounded-lg p-3.5 space-y-3">
      ...
      <div className="space-y-1">
        ...
        <MetricBar label="LLM TTFT" value={entry.llm_ttft_ms} max={maxBar} color="#6366f1" />
        <MetricBar label="LLM Total" value={entry.llm_total_ms} max={maxBar} color="#818cf8" />
        <MetricBar label="LLM 1st Sent." value={entry.llm_first_sentence_ms} max={maxBar} color="#a78bfa" />
        <MetricBar label="TTS Synth" value={entry.tts_synth_ms} max={maxBar} color="#8b5cf6" />
        <MetricBar label="TTS Wait" value={entry.tts_wait_ms} max={maxBar} color="#7c3aed" />
        <MetricBar label="1st Audio" value={entry.pipeline_first_audio_ms} max={maxBar} color="#10b981" />
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-0.5 pt-1 border-t border-border/40">
        ...
        {entry.sentences != null && <DetailRow label="Frases" value={fmtNum(entry.sentences)} />}
        {entry.audio_chunks != null && <DetailRow label="Audio chunks" value={fmtNum(entry.audio_chunks)} />}
        {entry.response_audio_ms != null && <DetailRow label="Audio resp." value={fmtMs(entry.response_audio_ms)} />}
        {entry.tool_rounds != null && entry.tool_rounds > 0 && <DetailRow label="Tool rounds" value={fmtNum(entry.tool_rounds)} />}
        {entry.session_duration_s != null && <DetailRow label="Sessão" value={`${entry.session_duration_s.toFixed(0)}s`} />}
        {entry.barge_in_count != null && entry.barge_in_count > 0 && <DetailRow label="Interrupções" value={fmtNum(entry.barge_in_count)} />}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-surface-light border border-border rounded-lg p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {last?.session_duration_s != null && (
          <span className="text-[10px] text-muted font-mono">
            Sessão: {last.session_duration_s.toFixed(0)}s
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <KpiCell label="LLM Total" value={fmtMs(avg("llm_total_ms"))} />
        <KpiCell label="TTS Synth" value={fmtMs(avg("tts_synth_ms"))} />
        <KpiCell label="1st Audio" value={fmtMs(avg("pipeline_first_audio_ms"))} />
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
          {metrics.length === 0 ? (
            <div className="text-center py-12 space-y-2">
              <p className="text-sm text-muted">Nenhuma métrica ainda.</p>
              <p className="text-xs text-muted/60">
                As métricas aparecem após cada resposta do agente.
              </p>
            </div>
          ) : (
            <>
