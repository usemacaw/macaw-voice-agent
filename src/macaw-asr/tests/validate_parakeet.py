"""Validate Parakeet model on GPU."""
...
config = EngineConfig(
    model_name="parakeet", model_id="nvidia/parakeet-tdt-0.6b-v3",
    device="cuda:0", dtype="float32", language="pt",
    audio=AudioConfig(input_sample_rate=16000, model_sample_rate=16000),
)

print("Creating parakeet...")
model = ModelFactory.create("parakeet")

print("Loading...")
t0 = time.perf_counter()
model.load(config)
print(f"Loaded in {(time.perf_counter()-t0)*1000:.0f}ms")
print(f"supports_streaming: {model.supports_streaming}")
print(f"supports_cuda_graphs: {model.supports_cuda_graphs}")
...
print(f"Text: {out.text!r} ({total:.0f}ms)")
...
print(f"Text: {out2.text!r} ({out2.timings.get('total_ms',0):.0f}ms)")
